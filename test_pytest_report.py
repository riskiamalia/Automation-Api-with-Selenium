...
def test_c_failed_login_with_invalid_email_valid_password(browser): 
    ...
    assert "Please include an '@' in the email"== respon_error_email
...
def test_i_failed_login_sqli_in_password(browser): 
    ...
    assert respon_msg== 'Tidak boleh mengandung symbol'

def test_j_failed_login_sqli_in_email(browser): 

    driver = browser 

    driver.get("http://barru.pythonanywhere.com/daftar")
    time.sleep(1)
    driver.find_element_by_xpath("/html/body/div/div[2]/form/input[1]").send_keys("SELECT%count%(*)%FROM%Users%WHERE%Username='jebol'%or%1=1%--%'%AND%Password=%'email'")
    time.sleep(1)
    driver.find_element_by_id("password").send_keys('sman60jakarta')
    time.sleep(1)
    driver.find_element_by_xpath('/html/body/div/div[2]/form/input[3]').click()
...
def test_a_failed_register_with_phone_password_empty(browser): 
    ...
    assert "Please include an '@' in the email"== error_email
